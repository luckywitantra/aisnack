const API_URL = "https://script.google.com/macros/s/AKfycbwRss8HzQwPardxTi4Scd-QOUZ2pitnsubY6pqASyLZA7oaagmym61VuFJvWjb91NRhfg/exec"; // <-- PASTIKAN GANTI DENGAN URL ANDA
...
const superApp = {
    ...
    // ==========================================
    // 1. FORMATTER (RUPIAH & WAKTU REGEX)
    // ==========================================
    ...
    parseDateId: function(dateStr) {
        ...
        if(s.includes('T')) { 
            let d = new Date(s); if (!isNaN(d.getTime())) { d.setHours(0,0,0,0); return d; } 
        }
    ...
    pullFreshData: async function() {
        ...
        try {
            ...
            if(data && data.status === 'sukses') {
                ...
                superApp.showToast("Data berhasil diperbarui dari Server!");
    ...
    apiPost: async function(payload) {
        if(!superApp.isOnline) {
            superApp.offlineQueue.push(payload); localStorage.setItem('aisnack_offline_queue', JSON.stringify(superApp.offlineQueue)); superApp.updateNetworkUI();
    ...
    syncOfflineQueue: async function() {
        ...
        superApp.showToast("Menyinkronkan data offline ke Server...", "warning");
        let failedQueue = [];
        for (let i = 0; i < superApp.offlineQueue.length; i++) {
            try { await fetch(API_URL, { method: 'POST', headers: { 'Content-Type': 'text/plain' }, body: JSON.stringify(superApp.offlineQueue[i]) }); } 
            catch(e) { failedQueue.push(superApp.offlineQueue[i]); }
        }
        ...
        if(superApp.offlineQueue.length === 0) {
            ...
            try { const res = await fetch(API_URL, { redirect: 'follow' }); superApp.db = await res.json(); superApp.refreshData(); } catch(e){}
        }
    ...
    updateNetworkUI: function() {
        ...
        if(superApp.isOnline) {
            if(superApp.offlineQueue.length > 0) {
                ind.className = 'flex items-center gap-2 px-3 py-1.5 rounded-full bg-orange-50 border border-orange-200 cursor-pointer transition'; dot.className = 'w-2 h-2 rounded-full bg-orange-500 animate-pulse'; txt.className = 'text-[10px] font-bold text-orange-600 hidden md:inline'; txt.innerText = `Menyinkron ${superApp.offlineQueue.length} data...`;
            } else {
            ...
            ind.className = 'flex items-center gap-2 px-3 py-1.5 rounded-full bg-red-50 border border-red-200 transition'; dot.className = 'w-2 h-2 rounded-full bg-red-500'; txt.className = 'text-[10px] font-bold text-red-600 hidden md:inline'; txt.innerText = `Offline (${superApp.offlineQueue.length} Pending)`;
        }
    },
    ...
    openCFD: async function() {
        try {
            if ('getScreenDetails' in window) {
                ...
                if (extScreen) { superApp.cfdWindow = window.open(window.location.href + '?mode=cfd', 'CFD_WINDOW', `left=${extScreen.availLeft},top=${extScreen.availTop},width=${extScreen.availWidth},height=${extScreen.availHeight},fullscreen=yes`); return; }
            }
        } catch(e) {}
    ...
    changePromoImage: function() {
        ...
        fileInput.onchange = (event) => {
            ...
            if(superApp.isProcessing) return; superApp.setLoading(true, "Mengunggah Promo...");
                ...
                img.onload = () => {
                    ...
                    superApp.apiPost(payload).then(res => {
                        if(res.status === 'sukses') {
                            localStorage.setItem('cfd_promo_url', base64); superApp.syncStorage(); superApp.setLoading(false); superApp.showToast("Banner Promo Diperbarui!");
                        }
    ...
    init: async function() {
        ...
        try {
            ...
            const logStat = document.getElementById('login-status');
            if(logStat && superApp.db) { logStat.innerText = 'Offline Mode Aktif (Gunakan PIN Anda)'; logStat.className = 'text-[10px] text-orange-500 font-bold uppercase tracking-widest text-center'; } 
    ...
    switchMenu: function(menu) {
        ...
        if(menu === 'report') superApp.renderReport(); if(menu === 'opname') superApp.renderOpname(); if(menu === 'audit') superApp.renderAudit();
        if(menu === 'terima') superApp.renderTerimaBarang(); if(menu === 'ai') superApp.generateAIReport(); if(menu === 'staf') superApp.renderStaf(); 
    },
