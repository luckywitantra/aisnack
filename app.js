/* === KONFIGURASI URL API === */
const API_URL = "https://script.google.com/macros/s/AKfycbwRss8HzQwPardxTi4Scd-QOUZ2pitnsubY6pqASyLZA7oaagmym61VuFJvWjb91NRhfg/exec";

/* === OBJEK SUPERAPP UTAMA === */
const superApp = {
    outlet: '',
    cart: [],
    printerChar: null,
    ...
    openVirtualNumpad: function(inputId, title, calcChange = false) {
        superApp.vnActiveInputId = inputId;
        superApp.vnIsCalcChange = calcChange;
        let titleEl = document.getElementById('vnumpad-title');
        if (titleEl) titleEl.innerText = title;
        let el = document.getElementById(inputId);
        let currentVal = el ? superApp.getNumericValue(el.value) : 0;
        let displayEl = document.getElementById('vnumpad-display');
        if (displayEl) displayEl.innerText = currentVal > 0 ? currentVal.toLocaleString('id-ID') : '0';
        const np = document.getElementById('virtual-numpad');
        if (np) np.classList.remove('translate-y-full');
    },
    closeVirtualNumpad: function() {
        const np = document.getElementById('virtual-numpad');
        if (np) np.classList.add('translate-y-full');
    },
    vnumpadAdd: function(val) {
        let display = document.getElementById('vnumpad-display');
        if (!display) return;
        let currentRaw = String(superApp.getNumericValue(display.innerText));
        if (currentRaw === '0') currentRaw = '';
        let newVal = currentRaw + val;
    ...
    vnumpadDel: function() {
        ...
        if (currentRaw.length > 1) {
            let newVal = currentRaw.slice(0, -1);
            display.innerText = parseInt(newVal, 10).toLocaleString('id-ID');
        } else {
            display.innerText = '0';
        }
    },
    vnumpadClear: function() {
        let disp = document.getElementById('vnumpad-display');
        if (disp) disp.innerText = '0';
    },
    vnumpadDone: function() {
        let display = document.getElementById('vnumpad-display');
        if (!display) return;
        let displayVal = display.innerText;
        let el = document.getElementById(superApp.vnActiveInputId);
        if (el) {
            el.value = displayVal === '0' ? '' : displayVal;
            if (superApp.vnIsCalcChange) {
                superApp.payCash = superApp.getNumericValue(displayVal);
                superApp.calcChange();
            }
    ...
    pullFreshData: async function() {
        ...
        superApp.setLoading(true, "Menarik Data Terbaru...");
        try {
            ...
            if (data && data.status === 'sukses') {
                ...
                superApp.showToast("Data berhasil diperbarui!");
            } else {
                throw new Error("Gagal");
            }
        } catch (e) {
            superApp.showToast("Gagal menarik data. Periksa internet.", "error");
        }
    ...
    showToast: function(msg, type = 'success') {
        ...
        t.className = `bg-white p-4 rounded-2xl shadow-2xl shadow-slate-200 flex items-center gap-3 toast-animate z-[999] dark:bg-slate-800 dark:border-slate-700 pointer-events-auto`;
    ...
    apiPost: async function(payload) {
        if (!superApp.isOnline) {
            superApp.offlineQueue.push(payload);
    ...
    syncOfflineQueue: async function() {
        ...
        superApp.showToast("Menyinkronkan data offline...", "warning");
        let failedQueue = [];
        for (let i = 0; i < superApp.offlineQueue.length; i++) {
            try {
                await fetch(API_URL, { method: 'POST', headers: { 'Content-Type': 'text/plain' }, body: JSON.stringify(superApp.offlineQueue[i]) });
            } catch (e) {
                failedQueue.push(superApp.offlineQueue[i]);
            }
        }
        superApp.offlineQueue = failedQueue;
        localStorage.setItem('aisnack_offline_queue', JSON.stringify(superApp.offlineQueue));
        if (superApp.offlineQueue.length === 0) {
            superApp.showToast("Semua data tersinkronisasi!");
            try {
                const res = await fetch(API_URL, { redirect: 'follow' });
                superApp.db = await res.json();
                superApp.refreshData();
            } catch (e) {}
        }
        superApp.updateNetworkUI();
    ...
    updateNetworkUI: function() {
        ...
        if (superApp.isOnline) {
            if (superApp.offlineQueue.length > 0) {
                ind.className = 'flex items-center gap-2 px-3 py-1.5 rounded-full bg-orange-50 border border-orange-200 cursor-pointer transition';
                dot.className = 'w-2 h-2 rounded-full bg-orange-500 animate-pulse';
                txt.className = 'text-[10px] font-bold text-orange-600 hidden md:inline';
                txt.innerText = `Menyinkron ${superApp.offlineQueue.length} data...`;
            } else {
                ind.className = 'flex items-center gap-2 px-3 py-1.5 rounded-full bg-green-50 border border-green-200 transition';
                dot.className = 'w-2 h-2 rounded-full bg-green-500';
                txt.className = 'text-[10px] font-bold text-green-600 hidden md:inline';
                txt.innerText = 'Online & Sinkron';
            }
        } else {
            ind.className = 'flex items-center gap-2 px-3 py-1.5 rounded-full bg-red-50 border border-red-200 transition';
            dot.className = 'w-2 h-2 rounded-full bg-red-500';
            txt.className = 'text-[10px] font-bold text-red-600 hidden md:inline';
            txt.innerText = `Offline (${superApp.offlineQueue.length} Pending)`;
        }
    },
    ...
    openCFD: async function() {
        try {
            if ('getScreenDetails' in window) {
                ...
                if (extScreen) {
                    superApp.cfdWindow = window.open(window.location.href + '?mode=cfd', 'CFD_WINDOW', `left=${extScreen.availLeft},top=${extScreen.availTop},width=${extScreen.availWidth},height=${extScreen.availHeight},fullscreen=yes`);
                    return;
                }
            }
        } catch (e) {}
    ...
    changePromoImage: function() {
        ...
        fileInput.onchange = (event) => {
            ...
            if (!file) return;
            if (superApp.isProcessing) return;
            superApp.setLoading(true, "Mengunggah Promo...");
            ...
            reader.onload = (e) => {
                ...
                img.onload = () => {
                    ...
                    superApp.apiPost(payload).then(res => {
                        if (res.status === 'sukses') {
                            localStorage.setItem('cfd_promo_url', base64);
                            superApp.syncStorage();
                            superApp.setLoading(false);
                            superApp.showToast("Banner Promo Diperbarui!");
                        }
                    }).catch(() => superApp.setLoading(false));
                };
    ...
    initCFD: function() {
        ...
        const cfdScreen = document.getElementById('cfd-screen');
        if (cfdScreen) cfdScreen.classList.remove('hidden');
        window.addEventListener('storage', (e) => {
            if (e.key === 'ai_snack_cfd' || e.key === 'cfd_promo_url') {
                let data = JSON.parse(localStorage.getItem('ai_snack_cfd') || '{}');
                if (data.outlet) superApp.renderCFD(data);
            }
        });
        let initialData = localStorage.getItem('ai_snack_cfd');
        if (initialData) superApp.renderCFD(JSON.parse(initialData));
    ...
    renderCFD: function(data) {
        const outNameEl = document.getElementById('cfd-outlet-name');
        if (outNameEl) outNameEl.innerText = `Cabang ${data.outlet}`;
        if (data.promoUrl) {
            const bg = document.getElementById('cfd-promo-bg');
            if (bg) bg.style.backgroundImage = `url('${data.promoUrl}')`;
        }
        const cfdStandby = document.getElementById('cfd-standby');
        const cfdSuccess = document.getElementById('cfd-success');
        if (data.status === 'paid') {
            cfdSuccess.classList.remove('hidden');
            document.getElementById('cfd-kembali').innerText = `Rp ${data.kembali.toLocaleString('id-ID')}`;
        ...
        if (data.items.length === 0 && data.status !== 'paid') {
            ...
            data.items.forEach(i => {
                html += `<div class="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex justify-between items-center"><div><h4 class="font-black text-slate-800 text-lg">${i.nama}</h4><p class="text-slate-500 font-bold">${i.qty} x Rp ${i.price.toLocaleString('id-ID')}</p></div><p class="font-black text-brand-500 text-xl">Rp ${(i.price * i.qty).toLocaleString('id-ID')}</p></div>`;
            });
            const listEl = document.getElementById('cfd-cart-list');
            if (listEl) listEl.innerHTML = html;
            const totEl = document.getElementById('cfd-total');
            if (totEl) totEl.innerText = `Rp ${data.total.toLocaleString('id-ID')}`;
        }
    },
    ...
    /* ========================================== */
    init: async function() {
        if (new URLSearchParams(window.location.search).get('mode') === 'cfd') {
            superApp.initCFD();
            return;
        }
        window.addEventListener('beforeunload', () => {
            if (superApp.cfdWindow && !superApp.cfdWindow.closed) superApp.cfdWindow.close();
        });
        window.addEventListener('online', () => {
            superApp.isOnline = true;
            superApp.syncOfflineQueue();
        });
        window.addEventListener('offline', () => {
            superApp.isOnline = false;
            superApp.updateNetworkUI();
        });
        
        try {
            let queue = localStorage.getItem('aisnack_offline_queue');
            superApp.offlineQueue = queue ? JSON.parse(queue) : [];
        } catch (e) {
            superApp.offlineQueue = [];
        }
        ...
        try {
            ...
            // Panggil API di background tanpa mengunci PIN
            let fetchPromise = (async () => {
                ...
                for (let i = 0; i < 3; i++) {
                    try {
                        ...
                        if (logStat && !superApp.db) logStat.innerText = `Mencoba ulang koneksi (${i+1}/3)...`;
                ...
                superApp.db = data;
                localStorage.setItem('aisnack_db_cache', JSON.stringify(data));
                let promoData = (superApp.db.pengaturan || []).find(x => x.Pengaturan === 'Promo_CFD');
    ...
    addPin: function(num) {
        if (superApp.pinBuffer.length < 4) {
            superApp.pinBuffer += num;
            const dot = document.getElementById(`dot-${superApp.pinBuffer.length}`);
            if (dot) {
    ...
    delPin: function() {
        if (superApp.pinBuffer.length > 0) {
            const dot = document.getElementById(`dot-${superApp.pinBuffer.length}`);
            if (dot) {
                dot.classList.replace('bg-brand-500', 'border-slate-300');
                dot.classList.replace('border-0', 'border-2');
            }
            superApp.pinBuffer = superApp.pinBuffer.slice(0, -1);
        }
    },
    clearPin: function() {
        superApp.pinBuffer = '';
    ...
    processLogin: function() {
        if (superApp.isProcessing) return;
        superApp.isProcessing = true;
        if (!superApp.db || !superApp.db.users) {
            superApp.showToast('Koneksi ke Database belum siap. Tunggu Sebentar.', 'error');
            superApp.clearPin();
        ...
        if (user) {
            superApp.currentUser = user;
            superApp.outlet = user.Outlet === 'Pusat' ? ((superApp.db.outlets || [])[0]?.ID_Outlet || 'Penajam') : user.Outlet;
            
            const sbRole = document.getElementById('sb-role');
            if (sbRole) sbRole.innerText = user.Role;
            const hInit = document.getElementById('header-initial');
            if (hInit) hInit.innerText = user.Username.charAt(0).toUpperCase();
            ...
            if (isAdmin) {
                ...
                let outOptions = '';
                let outFilters = '<option value="Semua">Semua Outlet</option>';
                ...
                if (selOut) {
                    selOut.innerHTML = outOptions;
                    selOut.value = superApp.outlet;
                    selOut.disabled = false;
                }
                if (repOut) repOut.innerHTML = outFilters;
                const btnPromo = document.getElementById('btn-ubah-promo');
                if (btnPromo) btnPromo.style.display = 'flex';
            } else {
                if (adminMenus) adminMenus.classList.add('hidden');
                if (selOut) {
                    selOut.classList.add('hidden');
                    selOut.innerHTML = `<option value="${superApp.outlet}">📍 ${superApp.outlet}</option>`;
            ...
            superApp.syncOfflineQueue();
            superApp.refreshData();
            superApp.checkShiftStatus();
            superApp.showToast(`Selamat datang, ${user.Username}!`);
        } else {
            superApp.showToast('PIN Tidak Dikenali', 'error');
            superApp.clearPin();
        }
        superApp.isProcessing = false;
    },
    ...
    checkShiftStatus: function() {
        ...
        if (shiftOutName) shiftOutName.innerText = superApp.outlet;
        ...
        if (openShift) {
            ...
            try {
                superApp.activeStaffTeam = JSON.parse(openShift.Tim_Operasional);
            } catch (e) {
                superApp.activeStaffTeam = [];
            }
            if (posView) posView.classList.remove('blur-lock');
        } else {
            superApp.activeShiftId = null;
            superApp.activeStaffTeam = [];
            ...
            if (staffListEl) staffListEl.innerHTML = staffHtml || '<p class="text-sm text-red-500">Tidak ada staf terdaftar di cabang ini.</p>';
    ...
    executeBukaShift: async function() {
        ...
        let m_awal = mAwalEl ? superApp.getNumericValue(mAwalEl.value) : 0;
        ...
        if (res.status === 'sukses') {
            superApp.activeShiftId = shiftID;
            superApp.activeStaffTeam = tim;
            if (res.is_offline) {
                let d = new Date();
                let pad = (n) => n < 10 ? '0' + n : n;
            ...
            superApp.closeModal('modal-shift');
            const posView = document.getElementById('view-pos');
            if (posView) posView.classList.remove('blur-lock');
    ...
    openKasKeluar: function() {
        const nom = document.getElementById('kas-out-nominal');
        if (nom) nom.value = '';
        const ket = document.getElementById('kas-out-ket');
        if (ket) ket.value = '';
        const mod = document.getElementById('modal-kas-keluar');
        const modc = document.getElementById('modal-kas-keluar-content');
        if (mod && modc) {
            mod.classList.remove('hidden');
            setTimeout(() => modc.classList.add('modal-enter-active'), 10);
        }
    },
    executeKasKeluar: async function() {
        if (superApp.isProcessing) return;
        let nomEl = document.getElementById('kas-out-nominal');
        let ketEl = document.getElementById('kas-out-ket');
        if (!nomEl || !ketEl) return;
        let nom = superApp.getNumericValue(nomEl.value);
        let ket = ketEl.value;
        ...
        if (res.status === 'sukses') {
            ...
            superApp.closeModal('modal-kas-keluar');
            superApp.showToast("Kas Keluar Tersimpan.");
            if (!res.is_offline) {
                const r = await fetch(API_URL, { redirect: 'follow' });
                superApp.db = await r.json();
                superApp.refreshData();
            }
        }
        superApp.setLoading(false);
    ...
    promptTutupShift: function() {
        const setAkhir = document.getElementById('shift-setoran-akhir');
        if (setAkhir) setAkhir.value = '';
        ...
        (superApp.db.kasKeluar || []).forEach(k => {
            if (k.ID_Shift === superApp.activeShiftId) totalKasKeluar += Number(k.Nominal);
        });

        let expected = modal + salesTunai - totalKasKeluar;

        const tMod = document.getElementById('ts-modal');
        if (tMod) tMod.innerText = `Rp ${modal.toLocaleString('id-ID')}`;
        const tSal = document.getElementById('ts-sales');
        if (tSal) tSal.innerText = `Rp ${salesTunai.toLocaleString('id-ID')}`;
        const tKas = document.getElementById('ts-kasout');
        if (tKas) tKas.innerText = `Rp ${totalKasKeluar.toLocaleString('id-ID')}`;
        const tExp = document.getElementById('ts-expected');
        if (tExp) tExp.innerText = `Rp ${expected.toLocaleString('id-ID')}`;

        const modalTutup = document.getElementById('modal-tutup-shift');
        const modalTutupContent = document.getElementById('modal-tutup-shift-content');
        if (modalTutup && modalTutupContent) {
            modalTutup.classList.remove('hidden');
            setTimeout(() => modalTutupContent.classList.add('modal-enter-active'), 10);
        }
    },
    executeTutupShift: async function() {
        if (superApp.isProcessing) return;
        let setAkhirEl = document.getElementById('shift-setoran-akhir');
        let setor = setAkhirEl ? superApp.getNumericValue(setAkhirEl.value) : 0;
    ...
    /* ========================================== */
    /* 6. POS CORE (RENDER, CART & FILTER)        */
    /* ========================================== */
    refreshData: function() {
        const hSub = document.getElementById('header-subtitle');
        if (hSub) hSub.innerText = `${superApp.outlet}`;
        const lOutManage = document.getElementById('label-outlet-manage');
        if (lOutManage) lOutManage.innerText = superApp.outlet;
    ...
    filterProducts: function(key) {
        ...
        if (pList) {
            if (superApp.isLoadingData) return;
            pList.innerHTML = superApp.filteredProducts.filter(p => String(p.nama || '').toLowerCase().includes(key.toLowerCase())).map(p => superApp.createProductCard(p)).join('');
        }
    },
    renderProducts: function() {
        const list = document.getElementById('product-list');
        if (!list) return;
        if (superApp.isLoadingData) {
    ...
    addToCart: function(sku, nama, price, maxStok, skuBahan, event) {
        ...
        superApp.cart.forEach(i => {
            if ((i.sku_bahan || i.sku) === refBahan) currentStokBahanDiKeranjang += i.qty;
        });
        ...
        if (event) {
            ...
            if (cartIcon) {
                ...
                requestAnimationFrame(() => {
                    dot.style.transform = `translate(${rect.left - event.clientX}px, ${rect.top - event.clientY}px) scale(0.5)`;
                    dot.style.opacity = '0';
                });
        ...
        if (item) item.qty++;
        else superApp.cart.push({ sku, nama, price, qty: 1, sku_bahan: skuBahan, maxStok: maxStok });
        superApp.renderCart();
        setTimeout(() => {
    ...
    changeQty: function(idx, val) {
        superApp.cart[idx].qty += val;
        if (superApp.cart[idx].qty <= 0) superApp.cart.splice(idx, 1);
        superApp.renderCart();
    },
    renderCart: function() {
        const cont = document.getElementById('cart-container');
        ...
        superApp.cart.forEach((i, idx) => {
            ...
            superApp.cart.forEach(c => {
                if ((c.sku_bahan || c.sku) === refBahan) sisaBahanDiKeranjang += c.qty;
            });
    ...
    openPaymentModal: function() {
        ...
        const pt = document.getElementById('pay-total');
        if (pt) pt.innerText = `Rp ${superApp.payTotal.toLocaleString('id-ID')}`;
        superApp.setPaymentMethod('Tunai');
        superApp.setCash('');
        const mp = document.getElementById('modal-payment');
        const mpc = document.getElementById('modal-payment-content');
        if (mp && mpc) {
            mp.classList.remove('hidden');
            setTimeout(() => { mpc.classList.add('modal-enter-active'); }, 100);
        }
    },
    setPaymentMethod: function(method) {
        ...
        const sectTunai = document.getElementById('tunai-section');
        if (method === 'Tunai') {
    ...
    setCash: function(val) {
        ...
        if (input) {
            if (val === 'pas') {
                input.value = superApp.payTotal.toLocaleString('id-ID');
                superApp.payCash = superApp.payTotal;
            } else if (val === 0 || val === '') {
                input.value = '';
                superApp.payCash = 0;
            } else {
                input.value = val.toLocaleString('id-ID');
                superApp.payCash = val;
            }
        }
        superApp.calcChange();
    },
    calcChange: function() {
        ...
        if (changeEl && btn) {
            if (superApp.payChange < 0) {
                changeEl.innerText = `Kurang Rp ${Math.abs(superApp.payChange).toLocaleString('id-ID')}`;
                changeEl.classList.replace('text-slate-800', 'text-red-500');
                btn.disabled = true;
                btn.classList.add('opacity-50', 'cursor-not-allowed');
            } else {
                changeEl.innerText = `Rp ${superApp.payChange.toLocaleString('id-ID')}`;
    ...
    executeCheckout: async function() {
        ...
        if (res.status === 'sukses') {
            ...
            if (res.is_offline) {
                ...
                superApp.db = await refreshRes.json();
            }
            superApp.refreshData();
        }
        superApp.cart = [];
        superApp.renderCart();
        superApp.closeModal('modal-payment');
        superApp.setLoading(false);
    },
    ...
    renderTerimaBarang: function() {
        ...
        if (lbl) lbl.innerText = superApp.outlet;
        ...
        if (tU) tU.innerHTML = hu || superApp.getEmptyState('fa-box-open', 'Belum Ada Bahan', 'Tambahkan bahan di menu gudang');
        const tP = document.getElementById('terima-tbody-pendukung');
        if (tP) tP.innerHTML = hp || superApp.getEmptyState('fa-box-open', 'Belum Ada Barang', 'Tambahkan pendukung di gudang');
    ...
    submitTerimaBarang: async function() {
        ...
        if (items.length === 0) {
            superApp.setLoading(false);
            return superApp.showToast("Tidak ada barang masuk yang diinput!", "error");
        }
    ...
    renderOpname: function() {
        ...
        if (mobCards) mobCards.innerHTML = `<h4 class="font-extrabold text-brand-600 mt-2 mb-2 bg-brand-50 p-3 rounded-xl border border-brand-100 text-sm">A. Bahan Utama</h4>` + (hum || '<p class="text-xs text-center">Kosong</p>') + `<h4 class="font-extrabold text-slate-600 mt-6 mb-2 bg-slate-100 p-3 rounded-xl border border-slate-200 text-sm">B. Pendukung & Packaging</h4>` + (hpm || '<p class="text-xs text-center">Kosong</p>');
    },
    calcOpname: function(sku) {
        const sysEl = document.getElementById(`opn-sys-${sku}`);
        let sys = parseInt(sysEl ? sysEl.innerText : 0) || 0;
        let fisikEl = document.getElementById(`opn-fisik-${sku}`);
        let fisik = superApp.getNumericValue(fisikEl ? fisikEl.value : 0);
        let selEl = document.getElementById(`opn-selisih-${sku}`);
        if (!selEl) return;
        if (isNaN(fisik) || (fisikEl && fisikEl.value === '')) {
            selEl.innerText = '-';
            selEl.className = 'py-3 px-4 text-right font-black text-slate-300';
            return;
        }
    ...
    calcOpnameMob: function(sku) {
        const sysEl = document.getElementById(`opn-sys-mob-${sku}`);
        let sys = parseInt(sysEl ? sysEl.innerText : 0) || 0;
        let fisikEl = document.getElementById(`opn-fisik-mob-${sku}`);
        let fisik = superApp.getNumericValue(fisikEl ? fisikEl.value : 0);
        let selEl = document.getElementById(`opn-selisih-mob-${sku}`);
        if (!selEl) return;
        if (isNaN(fisik) || (fisikEl && fisikEl.value === '')) {
            selEl.innerText = '-';
            selEl.className = 'font-black text-slate-300 text-lg';
            return;
        }
        let selisih = fisik - sys;
        selEl.innerText = selisih > 0 ? `+${selisih}` : selisih;
    ...
    submitOpname: async function() {
        ...
        if (items.length === 0) {
            superApp.setLoading(false);
            return superApp.showToast("Tidak ada stok yang dihitung!", "error");
        }
        ...
        if (tbodyTr) {
            ...
            (superApp.db.mutasi || []).forEach(mt => {
                if (mt.Status_Approval === 'Pending') {
                    ...
                    let wStr = safeWaktu.includes('T') ? superApp.cleanDateOnly(safeWaktu) + ' ' + superApp.cleanTimeOnly(safeWaktu) : safeWaktu;
    ...
    processBulkApproval: async function(status) {
        ...
        try {
            ...
            superApp.showToast("Gagal memproses", "error");
        }
        superApp.setLoading(false);
    },
    ...
    openTransferModalOwner: function() {
        let outletOpts = '';
        (superApp.db.outlets || []).forEach(o => { outletOpts += `<option value="${o.ID_Outlet}">${o.Nama_Outlet}</option>`; });
        let opt = '';
        [...(superApp.db.masterProduk || [])].sort((a, b) => String(a.Nama_Produk || '').localeCompare(String(b.Nama_Produk || ''))).forEach(m => {
            if (String(m.Kategori || '').toLowerCase() === 'bahan' || String(m.Kategori || '').toLowerCase() === 'pendukung') {
    ...
    executeTransferOwner: async function() {
        ...
        let sku = elSku.value;
        let qty = superApp.getNumericValue(elQty.value);
        let targetOutlet = elTujuan.value;
        let asalOutlet = elAsal ? elAsal.value : superApp.outlet;
        ...
        if (res.status === 'sukses') {
            ...
            superApp.showToast("Transfer dikirim! Menunggu Penerimaan di toko tujuan.");
            if (!res.is_offline) {
                const r = await fetch(API_URL, { redirect: 'follow' });
                superApp.db = await r.json();
            }
    ...
    openRestokModal: function() {
        ...
        setTimeout(() => { 
            let rstkInput = document.getElementById('frm-rstk-qty');
            if(rstkInput) {
                rstkInput.setAttribute('readonly', 'readonly');
                rstkInput.classList.add('cursor-pointer');
                rstkInput.onclick = () => superApp.openVirtualNumpad('frm-rstk-qty', 'Jumlah Beli', false);
            }
        }, 100);
    },
    ...
    openDistribusiModal: function(prefillSku = '', prefillOutlet = '') {
        ...
        setTimeout(() => { 
            let distInput = document.getElementById('frm-dist-qty');
            if(distInput) {
                distInput.setAttribute('readonly', 'readonly');
                distInput.classList.add('cursor-pointer');
                distInput.onclick = () => superApp.openVirtualNumpad('frm-dist-qty', 'Jumlah Kirim', false);
            }
        }, 100);
    },
