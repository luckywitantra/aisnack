...
const superApp = {
    ...
    openVirtualNumpad: function(inputId, title, calcChange = false) {
        this.vnActiveInputId = inputId; this.vnIsCalcChange = calcChange;
        let titleEl = document.getElementById('vnumpad-title'); if (titleEl) titleEl.innerText = title;
        let el = document.getElementById(inputId); let currentVal = el ? this.getNumericValue(el.value) : 0;
    ...
    vnumpadAdd: function(val) {
        ...
        let currentRaw = String(this.getNumericValue(display.innerText)); if (currentRaw === '0') currentRaw = '';
        let newVal = currentRaw + val; if (newVal.length > 11) return;
        display.innerText = parseInt(newVal, 10).toLocaleString('id-ID');
    },
    vnumpadDel: function() {
        let display = document.getElementById('vnumpad-display'); if (!display) return;
        let currentRaw = String(this.getNumericValue(display.innerText));
    ...
    vnumpadDone: function() {
        let display = document.getElementById('vnumpad-display'); if (!display) return;
        let displayVal = display.innerText; let el = document.getElementById(this.vnActiveInputId);
        if (el) {
            el.value = displayVal === '0' ? '' : displayVal;
            if (this.vnIsCalcChange) { this.payCash = this.getNumericValue(displayVal); this.calcChange(); }
            el.dispatchEvent(new Event('input', { bubbles: true }));
        }
        this.closeVirtualNumpad();
    },
    ...
    syncOfflineQueue: async function() {
        ...
        this.showToast("Menyinkronkan data...", "warning"); let failedQueue = [];
    ...
    updateNetworkUI: function() {
        ...
        } else { ind.className = 'flex items-center gap-2 px-3 py-1.5 rounded-full bg-red-50 border border-red-200 transition'; dot.className = 'w-2 h-2 rounded-full bg-red-500'; txt.className = 'text-[10px] font-bold text-red-600 hidden md:inline'; txt.innerText = `Offline (${this.offlineQueue.length})`; }
    },
    ...
    renderAudit: function() {
        ...
        if (tbodyOp) {
            ...
            tbodyOp.innerHTML = html || `<tr><td colspan="8" class="text-center py-6 h-32">${this.getEmptyState('fa-clipboard-check', 'Audit Bersih', 'Tidak ada laporan opname yang pending')}</td></tr>`;
        }
        ...
        if (tbodyTr) {
            ...
            tbodyTr.innerHTML = html || `<tr><td colspan="7" class="text-center py-6 h-32">${this.getEmptyState('fa-box-open', 'Audit Bersih', 'Tidak ada penerimaan barang yang pending')}</td></tr>`;
        }
        this.checkBulkAudit();
    },
    ...
    openTransferModalOwner: function() {
        ...
        setTimeout(() => {
            ...
            if (trfInput) { trfInput.setAttribute('readonly', 'readonly'); trfInput.classList.add('cursor-pointer'); trfInput.onclick = () => superApp.openVirtualNumpad('frm-trf-qty', 'Jumlah Kirim', false); }
    ...
    openRestokModal: function() {
        ...
        setTimeout(() => { 
            ...
            if(rstkInput) {
                ...
                rstkInput.onclick = () => this.openVirtualNumpad('frm-rstk-qty', 'Jumlah Beli', false);
            }
        }, 100);
    },
    ...
    openDistribusiModal: function(prefillSku = '', prefillOutlet = '') {
        ...
        setTimeout(() => { 
            ...
            if(distInput) {
                ...
                distInput.onclick = () => this.openVirtualNumpad('frm-dist-qty', 'Jumlah Kirim', false);
            }
        }, 100);
    },
